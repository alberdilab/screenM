...
def main():
    ap = argparse.ArgumentParser(
        description=(
            "Merge many sample-specific JSON files into one JSON.\n\n"
            "Each input JSON must contain a 'sample' field. The output is:\n"
            "{\n"
            "  'n_samples': N,\n"
            "  'samples': {\n"
            "      '<sample1>': { ... },\n"
            ...
            "  }\n"
            "}\n\n"
            "All per-sample structure is kept as-is; only 'sample' is removed "
            "from the nested object (it's used as the key instead)."
        )
    )
    ap.add_argument(
        "-i", "--input",
